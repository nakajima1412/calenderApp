...
export default {
  ...
  methods: {
    async getEvents () {
      let snapshot = await FirebaseManager.fetchFromFirebase()
    ...
    async addEvent () {
      if (this.name && this.start && this.end) {
        await FirebaseManager.commitToFirebase({
          name: this.name,
    ...
    async updateEvent (ev) {
      await FirebaseManager.updateFromFirebase(ev)
      this.selectedOpen = false
      this.currentlyEditing = null
    },
    async deleteEvent (ev) {
      await FirebaseManager.deleteFromFirebase(ev)
      this.selectedOpen = false
      this.getEvents()
    },
    viewDay ({ date }) {
